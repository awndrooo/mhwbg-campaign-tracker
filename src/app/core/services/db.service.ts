...
// OBJECT STORES
const HUNTER_PROFILE_STORE_NAME = 'HunterProfiles';

export const IDB_STORE = (storeName: string) =>
...
export const IDB_PUT = <T>(object: T) =>
  pipe<
...
export const IDB_ADD = <T>(object: T) =>
  pipe<
    Observable<IDBObjectStore>,
    Observable<IDBRequest>,
    Observable<IDBValidKey>
  >(
    map((objectStore) => objectStore.add(object)),
    transactionToObservable<IDBValidKey>()
  );

export const IDB_DELETE = (hunterId: string) =>
  pipe<
    Observable<IDBObjectStore>,
    Observable<IDBRequest>,
...
export class DbService {
  ...
  constructor(private _store$: Store) {
    this._connect(environment.IDBName, environment.IDBVersion);
  }
